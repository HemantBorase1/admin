  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Failed to load products and farmers. Please try again.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
...
const useProductFiltering = (products) => {
  ...
  const tabCounts = useMemo(() => {
    ...
    return { vegetablesCount, grainsCount, dairyCount };
  }, [filteredProducts]);
...
const useProductForm = (initialData = {}) => {
  ...
  const setFormDataFromProduct = useCallback((product) => {
    setFormData({
      name: product.name || "",
  ...
  const isFormValid = useMemo(() => {
    return formData.name.trim() && formData.description.trim() && formData.price > 0;
...
const ProductRow = ({ product, farmers, onView, onEdit, onDelete }) => {
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete the organic product from the catalog.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
...
const ProductDetailDialog = ({ product, farmers, open, onOpenChange }) => {
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{product.expiryDate}</p>
                </div>
              </div>
              <div>
...
const EditProductDialog = ({ product, open, onOpenChange, onSave }) => {
      ...
      <DialogContent className="max-w-lg min-w-[350px] max-h-[90vh] overflow-y-auto z-50">
        ...
        <div className="grid gap-4">
          ...
          <div>
            ...
            <Input name="unit" value={editForm.unit} onChange={handleChange} />
          </div>
          <div>
            <label className="text-sm font-medium">Quantity</label>
            <Input name="quantity" type="number" value={editForm.quantity} onChange={handleChange} />
          </div>
          <div>
            <label className="text-sm font-medium">Harvest Date</label>
            <Input name="harvest_date" type="date" value={editForm.harvest_date} onChange={handleChange} />
          </div>
          <div>
            <label className="text-sm font-medium">Expiry Date</label>
            <Input name="expiry_date" type="date" value={editForm.expiry_date} onChange={handleChange} />
          </div>
          <div>
            <label className="text-sm font-medium">Status</label>
            <Input name="status" value={editForm.status} onChange={handleChange} />
          </div>
          <div>
            <label className="text-sm font-medium">Image URL</label>
            <Input name="image_url" value={editForm.image_url} onChange={handleChange} />
          </div>
        </div>
...
const ProductStats = ({ products }) => {
  const stats = useMemo(() => {
    ...
    return { totalProducts, availableCount, lowStockCount, avgRating };
  }, [products]);
...
export function OrganicProductsManagement() {
  ...
  const { products, farmers, loading, updateProduct, deleteProduct } = useProductManagement();
  const { 
    searchTerm, 
    setSearchTerm, 
    activeTab, 
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
            <TabsList>
              ...
              <TabsTrigger value="vegetables">
                Vegetables ({tabCounts.vegetablesCount})
              </TabsTrigger>
              <TabsTrigger value="grains">
                Grains ({tabCounts.grainsCount})
              </TabsTrigger>
              <TabsTrigger value="dairy & others">
                Dairy & Others ({tabCounts.dairyCount})
              </TabsTrigger>
            </TabsList>
          </Tabs>
